...
module.exports = async function handler(req, res) {
  ...
  if (!session) return;
  try {
    const [projects, tasks, checklist, comments, log, recurring, recurring_log] = await Promise.all([
    ...
    return json(res, 200, {
      role: session.role, projects, tasks, checklist, comments, log, recurring, recurring_log,
    });
  } catch (e) {
